...
S9(완성본)는 일부러 mp4 를 굽지 않는다 — "재생하며 화면녹화하라"가 설계다.
이 단계는 그 대신 **결정론적으로** 만든다. 실시간 재생을 녹화하는 대신, 장마다

    1) `/preview/{pid}?n=` 로 그 장 화면만 시작 문 없이 정지 캡처하고
    2) 이미 나와 있는 내레이션 오디오 **길이만큼** 그 화면을 보여 주는 조각을
       ffmpeg 로 만들고
    3) 조각을 번호 순서로 이어 붙인다.

실시간 녹화(재생 속도만큼 걸리고, 화면-오디오 동기가 타이밍에 달림)보다 빠르고
어긋날 일이 없다 — 조각 길이 자체가 오디오 길이이기 때문이다.
