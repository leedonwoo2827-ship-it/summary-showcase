...
def run(job, pid: int, slug: str, project: Dict[str, Any], *, force: bool = False):
    ...
    for s in slides:
        ...
        # ★ **원고가 발음까지 갈라 주면 그것을 쓴다.** `data-read` 에는 숫자·약어가
        #   이미 풀려 있다(`1929년` → `천구백이십구 년`). 원고 쪽에서 소리 나는
        #   대로 적어 보내므로 여기서 다시 만들면 그 손질이 통째로 날아간다.
        #   **사람이 단추를 눌러 만드는 것이 아니라 처음부터 이렇게 나와야 한다**
        #   (2026-08-17 지시). 없으면 예전처럼 자막에서 만든다.
        ...
        read = clean(s.get("read"))
        spoken = read or honorific.to_polite(t)
