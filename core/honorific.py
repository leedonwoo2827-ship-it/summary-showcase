...
#   `장`도 넣으면 안 된다 — 이 앱에서 `19장`은 책의 章이라 「십구 장」이다.
# ★ **`배`가 빠져 있었다**(2026-09-06 지적: "3배 → 삼 배 아니고 세 배").
#   배수는 고유어로 센다 — 「세 배」·「네 배」·「열네 배」다. 사람이 발음 대본에서
#   손으로 고쳐 쓰고 있었다. 뒤따라 자주 쓰는 고유어 단위도 같이 넣는다.
# ★ 「장난」이 섞여 있던 것을 뺐다 — `3장난…` 같은 글은 없어서 한 번도 안 걸렸다.
#   `장`을 넣으려다 뒷글자가 붙은 자국으로 보인다(a593a70).
# ★ 긴 것을 앞에 둔다. 짧은 것이 먼저 걸려도 아래 「단위가 거기서 끝나야 한다」
#   검사에서 떨어져 나가지만, 순서를 지키면 헛걸음이 준다.
_NATIVE_UNIT = ("시간", "차례", "군데", "켤레", "가지", "그루", "송이", "봉지",
                "상자", "자루", "갈래", "마리", "사람",
                "개", "명", "번", "살", "곳", "달", "권", "배", "쪽", "줄",
                "칸", "판", "대", "채", "벌", "쌍", "병", "잔", "척", "통", "시")
